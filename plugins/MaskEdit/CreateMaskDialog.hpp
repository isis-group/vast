...
private:
	...
	std::shared_ptr<ImageHolder> _createEmptyMask( std::shared_ptr<ImageHolder> refImage ) {
		const util::fvector3 &refVoxelSize = refImage->getISISImage()->getValueAs<util::fvector3>( "voxelSize" ) +
											 ( refImage->getISISImage()->hasProperty( "voxelGap" ) ? refImage->getISISImage()->getValueAs<util::fvector3>( "voxelGap" ) : util::fvector3() );
		std::shared_ptr< ImageHolder > retImage;
		...
		mask.setValueAs<util::fvector3>( "voxelSize", voxelSize );
		mask.updateOrientationMatrices();

		if( mask.hasProperty( "Vista/ca" ) ) {
			std::list<double> ca = util::stringToList<double>( mask.getValueAs<std::string>( "Vista/ca" ) );
			...
			mask.setValueAs<std::string>( "Vista/ca", util::listToString<std::list<double>::iterator>( newCa.begin(), newCa.end(), " ", "", "" ) );
		}

		if( mask.hasProperty( "Vista/cp" ) ) {
			std::list<double> cp = util::stringToList<double>( mask.getValueAs<std::string>( "Vista/cp" ) );
			...
			mask.setValueAs<std::string>( "Vista/cp", util::listToString<std::list<double>::iterator>( newCp.begin(), newCp.end(), " ", "", "" ) );
		}

		mask.setValueAs<util::fvector3>( "indexOrigin", refImage->getISISImage()->getValueAs<util::fvector3>( "indexOrigin" ) );

		if( m_Interface.maskName->text().size() ) {
			mask.setValueAs<std::string>( "source", m_Interface.maskName->text().toStdString() );
		} else {
			mask.setValueAs<std::string>( "source", "mask" );
		}
