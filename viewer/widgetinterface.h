...
public:
	...
	virtual void addImage( const std::shared_ptr<ImageHolder>  ) {};
	virtual bool removeImage( const std::shared_ptr< ImageHolder >  ) { return true; };
	...
	void setWidgetEnsemble( std::shared_ptr<WidgetEnsemble> ensemble ) { m_WidgetEnsemble = ensemble; }
	const std::shared_ptr<WidgetEnsemble> getWidgetEnsemble() const { return m_WidgetEnsemble; }
	std::shared_ptr<WidgetEnsemble> getWidgetEnsemble() { return m_WidgetEnsemble; }

	std::string widget_file;

	//signals
	boost::signals2::signal<void ( const std::shared_ptr<WidgetEnsemble>, MouseButton ) > emitOnEnsembleClicked;
	boost::signals2::signal<void ( const std::shared_ptr<WidgetEnsembleComponent>, MouseButton ) > emitOnEnsembleComponentCLicked;
...
protected:
	...
	QWidget *m_Parent;
	std::shared_ptr<WidgetEnsemble> m_WidgetEnsemble;
